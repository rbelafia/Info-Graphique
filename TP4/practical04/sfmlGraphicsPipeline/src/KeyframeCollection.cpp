# include "../include/KeyframedCollection.hpp"
...
glm::mat4 KeyframeCollection::interpolateTransformation( float time ) const
{
    ...
    if( !m_keyframes.empty() )
    {
        ...
        // test
        orientation = glm::slerp(result[0].second.getOrientation(),result[1].second.getOrientation(), factor);
        translation = glm::lerp(result[0].second.getTranslation(),result[1].second.getTranslation(), factor);
        scale = glm::lerp(result[0].second.getScale(),result[1].second.getScale(), factor);
        //fin test
