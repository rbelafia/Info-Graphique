...
void getUnitCylinder(std::vector<glm::vec3> &positions, std::vector<glm::vec3> &normals, const unsigned int &slices)
{
	...
	float previous_angle_norm = (slices - 1) * angle_step;
    float angle = angle_step;
	float angle_norm = angle_step/2;
    float previous_sin = std::sin( previous_angle_norm );
    float previous_cos = std::cos( previous_angle_norm );
    float previous_sin_norm = std::sin( previous_angle );
    float previous_cos_norm = std::cos( previous_angle );
    float sin = 0;
    float cos = 1;
	float sin_norm = 0;
    float cos_norm = 1;
    for( size_t i = 0; i < slices;
		...
		previous_sin_norm = sin_norm, previous_cos_norm = cos_norm,
            angle += angle_step, cos = std::cos( angle ), sin = std::sin( angle), 
		angle_norm += angle_step, cos_norm = std::cos( angle_norm ), sin_norm = std::sin( angle_norm) )
    {
        size_t voffset = 12 * i;
        ...
        normals[ voffset +  3 ] = glm::vec3{cos_norm,sin_norm,0};
        normals[ voffset +  4 ] = glm::vec3{previous_cos_norm,previous_sin_norm,0};
        normals[ voffset +  5 ] = glm::vec3{previous_cos_norm,previous_sin_norm,0};

        normals[ voffset +  6 ] = glm::vec3{cos_norm,sin_norm,0};
        normals[ voffset +  7 ] = glm::vec3{previous_cos_norm,previous_sin_norm,0};
        normals[ voffset +  8 ] = glm::vec3{cos_norm,sin_norm,0};
