...
vec3 computeSpotLight(SpotLight light, vec3 surfel_to_camera)
{
    ...
    // if(cos < light.innerCutOff) intensity = 0.0f;

    float cos_inner = dot(surfel_normal, -light.spotDirection);
    float cos_outer = 2.0f * dot(surfel_normal, -light.spotDirection);

    intensity = clamp( (cos - light.outerCutOff ) / ( light.innerCutOff - light.outerCutOff ), 0, 1 );
